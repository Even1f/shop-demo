  ...
  <div class="home-category" @mouseleave="categoryId = ''">
    <ul class="menu">
      <li :class="{ active: categoryId === item.id }" v-for="item in menuList" :key="item.id" @mouseenter="categoryId = item.id">
        ...
        <template v-else>
          <shop-skeleton width="60px" height="18px" style="margin-right: 5px" bg="rgba(255,255,255,0.2)"></shop-skeleton>
          <shop-skeleton width="40px" height="18px" style="margin-right: 5px" bg="rgba(255,255,255,0.2)"></shop-skeleton>
        </template>
      </li>
    </ul>
    <!-- 弹层 -->
...
import ShopSkeleton from '@/components/library/shop-skeleton.vue'
export default {
  name: 'HomeCategory',
  setup() {
